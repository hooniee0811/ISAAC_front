...
const AddProjectScreen = () => {
  ...
  console.log(userId);
  console.log(format(today, 'yyyy-MM-dd 00:00:00'));
  ...
  const [project, setProject] = useState<ProjectProp>({
    ...
    end: format(addDays(today, 1), 'yyyy-MM-dd 00:00:00'),
    details: '',
  });
  // const [project, setProject] = useState<ProjectProp>({
  //   id: uuid.v4(),
  //   user_id: userId,
  //   name: '지구온난화의 심각성에 대한 유튜브 영상 제작',
  ...
  async function generateTasksWithAI() {
    if (project.name === '') {
      ...
      let cp = {...project};
      cp.start = format(start, 'yyyy-MM-dd 00:00:00');
      cp.end = format(addDays(end, 1), 'yyyy-MM-dd 00:00:00');
      setProject(cp);
      setModalVisible(true);

      var inputText = `project name: ${project.name}, project goal: ${project.goal}, expected outcome: ${project.expectedOutcome}, project starts at: ${start}, project ends at: ${end}, project details: ${project.details}`;
      ...
      try {
        response = await chatGPT(
          [
            [
              'system',
              `The assistant's job is to generate and schedule specific tasks of a project. Name, goal, expected outcome, start date, end date, and details of project will be provided as an input. Response JSONArray like [{order: number, name: string, duration: number, description: string},{order: number, name: string, duration: number, description: string},...]. If the input project is in Korean, 'name' and 'description' of each task are to be in Korean. Otherwise, 'name' and 'description' of each task are to be in English. 'description' must include the specific process and information about the task. 'duration' means how long the task takes, and its unit is to be hours. Return only JSON Array. Remove pre-text and post-text.`,
            ],
            [
              'user',
